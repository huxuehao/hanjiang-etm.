...
import com.hanjiang.etm.question.factory.template.MultipleQuestionConfig;
 ...
 **/
@Service
public class MultipleQuestionParser extends QuestionParser {
    ...
    public MultipleQuestionParser(TQuestionClassifyService tQuestionClassifyService) {
    ...
    public Optional<ImportQuestion> doParser(MultipartFile file) throws IOException {
        ...
        try (Workbook workbook = getWorkbook(file)) {
            ...
            if (sheet.getLastRowNum() > 502) {
                throw new RuntimeException("多选题一次性导入最多500条");
            }
            ...
            for (int i = 2; i <= lastRowNum; i++) {
                ...
                MultipleQuestionConfig questionConfig = new MultipleQuestionConfig();
                questionConfig.setType(QuestionType.MULTIPLE.value);
                    ...
                    if (j <= defaultHeads.size() - 1) {
                        switch (j) {
                            ...
                            case 6:
                                ...
                                break;
                            case 7:
                                String[] answers = getStringCellValue(row, j, true).replace("，", ",").split(",");
                                questionConfig.setAnswer(Arrays.asList(answers));
                                break;
                        }
                    }
                    // 条件成立：选项列
            ...
            // 处理答案，将文字转成A、B、C...
            // answerNum为excel中用户设置的选项数量
            Row row = sheet.getRow(1); // 获取title行
            int answerNum = row.getLastCellNum() - defaultHeads.size();
            ...
            for (MultipleQuestionConfig config : configs) {
                for (int i = 0; i < config.getAnswer().size(); i++) {
                    String latter = answerMap.get(config.getAnswer().get(i));
                    if (latter != null) {
                        config.getAnswer().set(i, latter);
                    } else {
