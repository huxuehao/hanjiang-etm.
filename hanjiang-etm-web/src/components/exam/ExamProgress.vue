...
// 统计初始化
const initTitle = () => {
  examUserCounts(props.examId).then((res) => {
    allUser.value = res.data.data;
  });
  examUserCompletion(props.examId).then((res) => {
    Object.assign(completion, res.data.data);
  });
  examUserQualified(props.examId, props.score).then((res) => {
...
const download = () => {
  dataLoading.value = true;
  examProgressExport(props.examId)
    .then((response) => {
      const blob = response.data;
      const link = document.createElement("a");
      link.href = URL.createObjectURL(blob);
      link.download = props.examName + "_考试统计.xlsx";
      link.click();
    })
    .finally(() => {
      dataLoading.value = false;
        ...
        <el-table-column label="是否完成" prop="paperDone" align="center">
          <template #default="scope">
            <el-tag
              disable-transitions
              type="info"
              v-if="scope.row.paperDone === 0"
              >未完成</el-tag
            >
            <el-tag
              disable-transitions
              type="warning"
              v-else-if="scope.row.corrected === 0"
              >待批改</el-tag
            >
            <el-tag disable-transitions type="success" v-else>已完成</el-tag>
          </template>
        </el-table-column>
        <el-table-column
          label="考试时间"
        ...
        <el-table-column label="操作" width="150" align="center">
          <template #default="scope">
            <el-button
              ...
              type="text"
              icon="View"
              :disabled="scope.row.paperDone !== 1 || scope.row.corrected !== 1"
              >查看</el-button
            >
          </template>
        </el-table-column>
